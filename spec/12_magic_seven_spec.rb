...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when the random number is 26 and the previous step is 40' do
      subject(:game_twenty_six) { described_class.new(26) }

      it 'returns 14' do
        previous_step = 40
        result = game_twenty_six.subtract_random_number(previous_step)
  ...
  describe '#play' do
    ...
    context 'when the random number is 250' do
      subject(:game_two_fifty) { described_class.new(250) }

      it 'will return 7' do
        result = game_two_fifty.play
        expect(result).to eq(7)
      end
    end

    context 'when the random number is -15' do
      subject(:game_neg_fifteen) { described_class.new(-15) }

      it 'will return 7' do
        result = game_neg_fifteen.play
        expect(result).to eq(7)
      end
    end
  end
