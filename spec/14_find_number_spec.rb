...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      subject(:guessing_seven) { described_class.new(5, 9, number_guessing) }

      it 'returns 7' do
        expect(guessing_seven.make_guess).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:guessing_eight) { described_class.new(8, 9, number_guessing) }

      it 'returns 8' do
        expect(guessing_eight.make_guess).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:guessing_one) { described_class.new(0, 3, number_guessing) }

      it 'returns 1' do
        expect(guessing_one.make_guess).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:guessing_three) { described_class.new(3, 3, number_guessing) }

      it 'returns 3' do
        expect(guessing_three.make_guess).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    let(:random_num) { double('random_number', value: 6) }
    ...
    context 'when guess and random_number are equal' do
      ...
      subject(:game_good_guessing) { described_class.new(0, 9, random_num, 6) }
      ...
      it 'is game over' do
        expect(game_good_guessing).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      subject(:game_bad_guessing) { described_class.new(0, 9, random_num, 8) }

      it 'is not game over' do
        expect(game_bad_guessing).not_to be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      it 'updates min to the same value as max' do
        expect { close_guess_game.update_range }.to change { close_guess_game.min }.to be{ close_guess_game.max }
      end

      it 'does not update max' do
        expect { close_guess_game.update_range }.not_to change { close_guess_game.max }
      end
    end

    context 'when the answer is 1' do
      let(:number_range) { double('random_number', value: 1) }

      context 'when the guess is 2, min = 1, and max = 3' do
        subject(:close_to_min_game) { described_class.new(0, 3, number_range, 2) }

        it 'updates max to the same value as min' do
          expect { close_to_min_game.update_range }.to change { close_to_min_game.max }.to be{ close_to_min_game.min }
        end

        it 'does not update min' do
          expect { close_to_min_game.update_range }.not_to change { close_to_min_game.min }
        end
      end
    end
  end
